...
Create Table Flight(
		...
		CONSTRAINT FOREIGN KEY (flight_airline_operator) REFERENCES Airline(airline_code), 
		CONSTRAINT FOREIGN KEY (flight_airplane) REFERENCES Airplane(airplane_code),
		CONSTRAINT FOREIGN KEY (flight_dep_airport) REFERENCES Airport(airport_code),
		CONSTRAINT FOREIGN KEY (flight_arr_airport) REFERENCES Airport(airport_code)
);
...
Create Table Customer(
		...
		CHECK (DATALENGHT(customer_email)>6 && customer_email LIKE '%@%.%')
);
...
Create Table Booking(
		...
		CONSTRAINT FOREIGN KEY (booking_agency) REFERENCES Agency(agency_name)
);
ALTER Table Booking 
ADD CONSTRAINT FOREIGN KEY (booking_airline_code) REFERENCES Airline (airline_code);
ALTER Table Booking
ADD CONSTRAINT FOREIGN KEY (booking_flight_number) REFERENCES Flight(flight_number);
ALTER Table Booking 
ADD CONSTRAINT FOREIGN KEY (booking_customer_id) REFERENCES Customer(customer_id);
